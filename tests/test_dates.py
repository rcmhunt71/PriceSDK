...
from APIs.dates.models.dates import DatesInfoKeys, Dates, DatesList, DatesKeys
...
def build_date_data():
    return {
        DatesInfoKeys.DATE_VALUE: f"{randrange(2005, 2018)}-{randrange(1, 12):02}-{randrange(1, 28):02}T"
                             f"{randrange(0, 24):02}:{randrange(0, 60):02}:{randrange(0, 60)}.{randrange(999):03}",
        DatesInfoKeys.DATE_NAME: f"Application {choice(status)}",
    }
...
class TestDates(unittest.TestCase, CommonResponseValidations):
    def test_date_model(self):
        ...
        date_model = Dates(**dates_data[index])
    ...
    def test_get_dates_response(self):
        ...
        dates_args[DatesKeys.DATES_LIST] = dates_data
        get_dates_resp = GetDatesResponse(**dates_args)
