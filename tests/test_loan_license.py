...
from APIs.loans.requests.set_loan_license_details import SetLoanLicenseDetailsRequestParams
...
class TestLoanLicenseClient(unittest.TestCase, CommonResponseValidations):
    ...
    def test_SetLoanLicenseData_client(self):
        ...
        param_str = "&".join([f"{getattr(SetLoanLicenseDetailsRequestParams, key.upper())}={value}" for key, value
                              in params.items()])
