...
from APIs.credits.responses.get_credit_agency_list import GetCreditAgencyListResponse
...
class ApiEndpoints:
    ...
    GET_CREDIT_AGENCY_LIST: str = "get_credit_agency_list"
...
class CreditsClient(BaseClient):
    ...
    def get_credit_agency_list(self, session_id=None, nonce=None, pretty_print=False):
        request_model = SimpleRequestModel(session_id=self._get_session_id(session_id),
                                           nonce=self._get_nonce(nonce), pretty_print=pretty_print)

        return self.get(resource_endpoint=ApiEndpoints.GET_CREDIT_AGENCY_LIST,
                        response_model=GetCreditAgencyListResponse, params=request_model.as_params_dict)
